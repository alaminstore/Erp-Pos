...
{{--                    {{ auth()->check() ? auth()->user()->unreadNotifications()->count() : 0 }}--}}0
                </span>
            </a>
    ...
    <div class="navbar-toggleable-sm" id="collapse">
        ...
        <ul class="nav navbar-nav">
            <li class="nav-item dropdown">
                <h6 class="nav-link" style="margin: 0;">Zalo</h6>
                {{--<form action="{{ url('factory-dashboard') }}" style="padding-top: 10px" autocomplete="off">
                    ...
                    @else
                        {!! Form::select('factory_id', $factories, Session::get('factoryId'), ['class' => 'form-control c-select', 'onchange' => "this.form.submit()"]) !!}
                    @endif
                </form>--}}
            </li>
        </ul>
